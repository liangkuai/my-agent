...
2. 动态段落：tools / workspace / memories 三段内容来自 context 字典（由
   context.update_session_context() 填充），而非硬编码——工具列表从 TOOLS 定义
...
4. 显式拼接顺序：assemble_system_prompt 按 identity → tools → workspace →
   ...
   context.update_session_context()，不是自动发现——这样更可控，且一眼就能看清段落顺序。
...
# ── 静态模板 ───────────────────────────────────────────────────────────
# identity 是唯一不随运行时变化的静态片段。tools / workspace / memories
# 三段由 assemble_system_prompt 从 context 字典中动态取值并格式化，
# 无需在此处维护副本。
...
def assemble_system_prompt(context: dict) -> str:
    ...
    context 由 context.update_session_context() 构建，包含三个字段：
